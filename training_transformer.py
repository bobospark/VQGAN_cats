...
class TrainTransformer:
    ...
    # 목적 : some modules are supposed to get weight decay on some modules or not and in particular all the models
    #        which are in here so all the linear modules they are supposed to get the weight decay and layer norms
    #        and embedding layers and also the positional embedding they are supposed to not receive weight decay
    def configure_optimizers(self):
        decay, no_decay = set(), set()
        whitelist_weight_modules = (nn.Linear,)
        blacklist_weight_modules = (nn.LayerNorm, nn.Embedding)
        
        for mn, m in self.model.transformer.named_modules():  # GPT
        ...
        for epoch in range(args.epochs):
            with tqdm(range(len(train_dataset))) as pbar:
                for i, imgs in zip(pbar, train_dataset):
                    ...
                    logits = logits.to(device = args.device)
                    ...
                    #         # transformer_images = torch.cat((imgs[i], sampled_imgs.add(1).mul(0.5)[0]))
                    #         vutils.save_image(sampled_imgs, os.path.join(r"/workspace/VQGAN/transformer_results", f"transformer_{epoch}_{i}.jpg"), nrow=6)

                    
                    
            log, sampled_imgs = self.model.log_images(imgs[0][None])
            vutils.save_image(sampled_imgs, os.path.join(r"/workspace/VQGAN/transformer_results", f"transformer_{epoch}.jpg"), nrow = 6)
            # plot_image(log)
            torch.save(self.model.state_dict(), os.path.join("transformer_checkpoints", f"transformer_{epoch}.pt"))

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="VQGAN")
    parser.add_argument('--latent-dim', type=int, default=256, help='Latent dimension n_z.')
    parser.add_argument('--image-size', type=int, default=512, help='Image height and width.)')
    ...
    args.checkpoint_path = r"/workspace/VQGAN/checkpoints/vqgan_epoch_49.pt"
